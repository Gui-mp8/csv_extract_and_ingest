...
def main():
    ...
    con_postgre_db = psycopg2.connect(
        dbname='indicium_postgre', user='postgre', host='localhost', password='postgre', port='5092')
...
# Creating the MySQL Environ.
    try:
        con_mysql = mysql.connect(
            host='localhost', user='root', password='mysql', port='5091')

        db_name = 'indicium_challenge'

        create_mysql_db(con_mysql=con_mysql, db_name=db_name)
        # Variable that after the SGBD connection.
        con_mysql_db = mysql.connect(
            host='localhost', database=db_name, user='root', password='mysql', port='5091')
...
# Finishing the Challenge.
        the_goal(con_mysql_db=con_mysql_db,
                 path_final_result=path_final_result)
