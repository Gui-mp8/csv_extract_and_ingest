    ...
    try:
        ...
        for tabela in LISTA_TABELAS:
            if tabela != 'order_details':
                df = pd.read_csv(rf'{path_dir_postgre}/{tabela}/{_date}/{tabela}.csv',index_col=False,delimiter=',',encoding = 'utf8')
            ...
            # loop through the data frame
                ...
                # here %S means string values 
                    query = f"INSERT INTO indicium_challenge.{tabela} VALUES ({y})" #If you are adding values for all the columns of the table, you do not need to specify the column names in the SQL query
                ...
                df = pd.read_csv(rf'{path_csv_external_files}/{tabela}.csv',index_col=False,delimiter=',',encoding = 'utf8')
                print(f'\nDataframe {tabela} carregado')
...
def the_goal(con_mysql_db,path_final_result):
    ...
    try:
        ...
        file = csv.writer(open(f'{directory}/final_result.csv', 'w',encoding='utf-8'))
        for i in result:
            file.writerow(i)
